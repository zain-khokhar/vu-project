...
export default function QuizSetup({ quizData }) {
  ...
  const questionOptions = [10, 20, 50, 100, quizData.totalQuestions].filter(
    (val, idx, arr) => arr.indexOf(val) === idx && val <= quizData.totalQuestions
  );
      ...
      <div className="max-w-2xl mx-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                {timeOptions.map((option) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setSettings({ ...settings, timePerQuestion: option.value })
                    }
                    className={`py-3 px-4 rounded-lg font-semibold transition-all ${
        ...
        <div className="mt-6 bg-white rounded-xl p-6 shadow-lg border border-gray-100">
          ...
          <ul className="space-y-2 text-sm text-gray-600">
            ...
            <li className="flex items-start">
              <span className="text-indigo-600 mr-2">•</span>
              <span>You'll see detailed explanations after completing the quiz</span>
            </li>
          </ul>
        </div>
      </div>
